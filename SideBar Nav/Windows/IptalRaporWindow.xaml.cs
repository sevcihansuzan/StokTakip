...
{
    public partial class IptalRaporWindow : Window
    {
        private static readonly CultureInfo Culture = CultureInfo.GetCultureInfo("tr-TR");

        private readonly IReadOnlyList<RezIptal> cancellations;
        private readonly IDictionary<string, List<RezIptalDetay>> detailsByReservation;
            ...
            string? filterDescription)
        {
            ...
            this.detailsByReservation = detailsByReservation;
            this.selection = selection;
            this.periodDescription = periodDescription;
            this.filterDescription = filterDescription;
            Loaded += IptalRaporWindow_Loaded;
        }

        private void IptalRaporWindow_Loaded(object sender, RoutedEventArgs e)
        {
            lblRecordCount.Text = cancellations.Count.ToString("N0", Culture);
        ...
        private void GenerateReport()
        {
            ...
            try
            {
                string tempDirectory = Path.Combine(Path.GetTempPath(), "SideBarNavIptal");
                Directory.CreateDirectory(tempDirectory);

                string stamp = DateTime.Now.ToString("yyyyMMddHHmmssfff");

                string pdfPath = Path.Combine(tempDirectory, $"IptalRaporu_{stamp}.pdf");
                currentPdfPath = ReportPdfService.GenerateIptalReport(
                    cancellations,
                ...
                string excelPath = Path.Combine(tempDirectory, $"IptalRaporu_{stamp}.xlsx");
                var excelResult = ReportExcelService.GenerateIptalReport(
                    cancellations,
                    detailsByReservation,
                    selection,
                    periodDescription,
            ...
            {
                pdfBrowser.NavigateToString($"<html><body><p>İptal raporu oluşturulurken hata oluştu:<br/>{WebUtility.HtmlEncode(ex.Message)}</p></body></html>");
                UpdateExcelPreview(null);
                MessageBox.Show($"İptal raporu oluşturulurken hata oluştu:\n{ex.Message}", "Hata", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
        ...
        private void UpdatePdfPreview(string? sourcePath)
        {
            ...
            try
            {
                string tempPath = Path.Combine(Path.GetTempPath(), $"IptalRaporPreview_{Guid.NewGuid():N}.pdf");
        ...
        private void BtnDownloadPdf_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    MessageBox.Show("İptal raporu PDF dosyası kaydedildi.", "Bilgi", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch (Exception ex)
                {
        ...
        private void BtnDownloadExcel_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    MessageBox.Show("İptal raporu Excel dosyası kaydedildi.", "Bilgi", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Dosya kaydedilirken hata oluştu:\n{ex.Message}", "Hata", MessageBoxButton.OK, MessageBoxImage.Error);
